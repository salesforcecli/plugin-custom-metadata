...
describe('sfdx force:cmdt:record:insert', () => {
  ...
  fs.writeFileSync('badCSV/countries.csv', 'Label,CountryCode__c,CountryName__c\nAustralia,AU,Australia\n');
  ...
  test
    ...
    .it('fails force:cmdt:record:insert', (ctx) => {
      expect(/.*?no such file or directory, scandir.*?badCSV.*?Snapple__mdt.*?fields.*/g.test(ctx.stderr)).to.be.equal(
        true
      );
      fs.rmSync(secondTest, { recursive: true });
    });
