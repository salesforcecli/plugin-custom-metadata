...
export class MetadataUtil {
  ...
  public async describeObj(objName: string): Promise<AnyJson> {
    const result = await this.conn.metadata.read(
      'CustomObject',
      objName,
      (err, meta) => {
        if (err) {
  ...
  public async queryRecords(describeResult: AnyJson): Promise<AnyJson> {
    const query = await this._getSoqlQuery(
      describeResult['fields'],
      describeResult['fullName']
    );
  ...
  public validCustomSettingType(objDescribe: AnyJson): boolean {
    if (
      objDescribe['customSettingsType'] === 'List' &&
    ...
    Object.keys(sObjecRecord).forEach((fieldName) => {
      if (fieldName !== 'attributes' && fieldName !== 'Name') {
        ...
        if (fieldType === 'Location') {
          if (
            fieldValue.includes('latitude') ||
            fieldValue.includes('longitude')
          ) {
            record['Lat_' + fieldName] = fieldValue.slice(
              fieldValue.indexOf(':') + 1,
              fieldValue.indexOf(',')
            );
            record['Long_' + fieldName] = fieldValue.slice(
